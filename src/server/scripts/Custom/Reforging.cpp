...
namespace
{
    static const char* GetStatName(uint32 ItemStatType)
    {
        ...
        case ITEM_MOD_DODGE_RATING             	: return "　闪避等级　"; break;
        ...
        case ITEM_MOD_EXPERTISE_RATING         	: return "　熟练等级　"; break;
        default: return NULL;
        }
    }
...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "　选择需要重铸的物品　:", MAIN_MENU, 0);
        ...
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TRAINER, "　移除重铸　", SELECT_RESTORE, 0);
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "　刷新菜单　", MAIN_MENU, 0);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
    {
        ...
        case SELECT_STAT_REDUCE:
            ...
            if (Item* invItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, action))
            {
                if (IsReforgable(invItem, player))
                {
                    ...
                    const ItemTemplate* pProto = invItem->GetTemplate();
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "　属性降低　:", sender, action);
                    for (uint32 i = 0; i < pProto->StatsCount; ++i)
                    {
                    ...
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "　返回..　", MAIN_MENU, 0);
                    player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
                }
                else
                {
                    player->GetSession()->SendNotification("　无效的物品　　");
        ...
        case SELECT_STAT_INCREASE:
            // action = StatsCount id
            {
                ...
                if (invItem)
                {
                    ...
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "　属性增加　:", sender, action);
                    for (uint32 i = 0; i < stat_type_max; ++i)
                    {
                        ...
                        if (const char* stat_name = GetStatName(statTypes[i]))
                        {
                            ...
                            player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, oss.str(), pProto->ItemStat[action].ItemStatType, statTypes[i], "　你确定要重铸　\n\n"+pProto->Name1, (pProto->SellPrice < (10*GOLD) ? (10*GOLD) : pProto->SellPrice), false);
                        }
                    }
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "　返回　..", SELECT_STAT_REDUCE, invItem->GetSlot());
                    player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
                }
                else
                {
        ...
        case SELECT_RESTORE:
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "　选择需要移除重铸的物品　:", sender, action);
                for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; slot++)
                {
                    if (Item* invItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, slot))
                        if (sObjectMgr->_itemFakeStatStore.find(invItem->GetGUIDLow()) != sObjectMgr->_itemFakeStatStore.end())
                            if (const char* slotname = GetSlotName(slot, player->GetSession()))
                                player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, slotname, RESTORE, slot, "　确定要移除重铸　\n\n"+std::string(slotname), 0, false);
                }
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "　刷新菜单　", sender, action);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "　返回　..", MAIN_MENU, 0);
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            }
            break;
            ...
            // sender = stat type to decrease, action = stat type to increase
            {
                ...
                if (invItem && IsReforgable(invItem, player))
                {
                    if (player->HasEnoughMoney(invItem->GetTemplate()->SellPrice < (10*GOLD) ? (10*GOLD) : invItem->GetTemplate()->SellPrice))
                    {
                        ...
                        player->GetSession()->SendNotification("　没有足够的金币　");
                }
                else
                    player->GetSession()->SendNotification("　无效的物品　");
                OnGossipHello(player, creature);
