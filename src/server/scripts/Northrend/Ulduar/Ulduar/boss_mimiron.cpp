            ...
            void UpdateAI(uint32 diff)
            {
                ...
                if (_phase == PHASE_INTRO)
                {
                    if (_phaseTimer <= diff)
                    {
                        ...
                        {
                            case 1:
                                Talk(_mimironHardMode ? SAY_HARDMODE_ON : SAY_AGGRO);
...
public:
    ...
    struct npc_proximity_mineAI : public CreatureAI
    {
        npc_proximity_mineAI(Creature* creature) : CreatureAI(creature)
        {
