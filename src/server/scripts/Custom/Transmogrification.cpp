...
const char* Transmogrification::GetSlotName(uint8 slot, WorldSession* session) const
{
    ...
    case EQUIPMENT_SLOT_FEET: return  "脚部";// session->GetTrinityString(LANG_SLOT_NAME_FEET);
    case EQUIPMENT_SLOT_WRISTS: return  "手腕";// session->GetTrinityString(LANG_SLOT_NAME_WRISTS);
    case EQUIPMENT_SLOT_HANDS: return  "手部";// session->GetTrinityString(LANG_SLOT_NAME_HANDS);
    ...
    case EQUIPMENT_SLOT_TABARD: return  "圣物";// session->GetTrinityString(LANG_SLOT_NAME_TABARD);
    default: return NULL;
    }
}
