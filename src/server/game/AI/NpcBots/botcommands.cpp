...
#define GM_COMMANDS rbac::RBACPermissions(199)
...
public:
    ...
    static bool HandleNpcBotLookupCommand(ChatHandler* handler, const char* args)
    {
        ...
        {
            handler->PSendSysMessage("未知的职业 %u", uint32(botclass));
            handler->SetSentErrorMessage(true);
            return false;
        }
    ...
    static bool HandleNpcBotDeleteCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (!ubot)
        {
            handler->SendSysMessage(".npcbot shan");
            handler->SendSysMessage("从数据库中删除机器人");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (!bot || !bot->IsNPCBot())
        {
            handler->SendSysMessage("没有选中机器人.");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        handler->SendSysMessage("机器人被删除了.");
        return true;
    }
    ...
    static bool HandleNpcBotSpawnCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
        {
            ...
            handler->SendSysMessage("召唤一个机器人");
            handler->SendSysMessage("用法: .npcbot spawn #ID");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (res1)
        {
            handler->PSendSysMessage("机器人 %u 已经存在 `characters_npcbot` 表!", id);
            handler->SendSysMessage("想召唤请先删除那个机器人");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (chr->GetTransport())
        {
            handler->SendSysMessage("无法再运输器上召唤!");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (map->Instanceable())
        {
            handler->SendSysMessage("不能在副本里面召唤!");
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        {
            handler->SendSysMessage("数据库中找不到机器人!");
            handler->SetSentErrorMessage(true);
            //return false;
            delete creature;
            return false;
    ...
    static bool HandleNpcBotResetCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        handler->PSendSysMessage(".npcbot shuaxin");
        handler->PSendSysMessage("刷新机器人,只能在非战斗状态下使用.");
        handler->SetSentErrorMessage(true);
        return false;
    }
    static bool HandleNpcBotReviveCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (!u)
        {
            handler->SendSysMessage(".npcbot fuhuo");
            handler->SendSysMessage("复活机器人.");
            handler->SetSentErrorMessage(true);
            return false;
        }
